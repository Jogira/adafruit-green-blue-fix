...
class Sprite(displayio.TileGrid):
    def __init__(self, filename, transparent=None):
        ...
        # Swap green and blue color components in the palette
        for i in range(len(palette)):
            color = palette[i]
            red = (color >> 16) & 0xFF
            green = (color >> 8) & 0xFF
            blue = color & 0xFF
            swapped_color = (red << 16) | (blue << 8) | green
            palette[i] = swapped_color

        # Adjust transparent color if provided
        if isinstance(transparent, (tuple, list)):
            ...
            for color_index, color in enumerate(palette):
                delta = (
                    transparent_color[0] - ((color >> 16) & 0xFF),
                    transparent_color[1] - ((color >> 8) & 0xFF),
                    transparent_color[2] - (color & 0xFF)
                )
                rgb_distance = (
                    delta[0] * delta[0]
                    + delta[1] * delta[1]
